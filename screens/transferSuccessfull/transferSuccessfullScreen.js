...
class TransferSuccessfullScreen extends Component {
      ...
      <SafeAreaView style={{ flex: 1, backgroundColor: "#F2F4F6" }}>
        ...
        <View style={{ flex: 1 }}>
          <ScrollView showsVerticalScrollIndicator={false}>
            ...
            {this.transferInfo({
              title: "Transfer to",
              description: this.transferTo,
            })}
            {this.transferInfo({
              title: "From",
              description: "Saving | 0325 2365 1475",
            })}
            {this.transferInfo({
              title: "Date & Time",
              description: "18 April, 2021 | 11:45 AM",
            })}
            {this.transferInfo({
              title: "Amount",
              description: "$1045.00 USD",
            })}
            {this.transferInfo({
              title: "Payment mode",
  ...
  backtoHomeButton() {
    return (
      ...
      >
        <Text style={{ ...Fonts.whiteColor16Bold }}>Back to Home</Text>
      </TouchableOpacity>
    );
  }
  ...
  transferInfo({ title, description }) {
    return (
      ...
      >
        <Text style={{ ...Fonts.grayColor14Regular }}>{title}</Text>
